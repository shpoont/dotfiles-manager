...
Common `dotfiles-manager` commands:

```text
dotfiles-manager --version
dotfiles-manager version
dotfiles-manager completion <bash|fish|powershell|zsh> [--no-descriptions]
...
dotfiles-manager [--config <.dotfiles-manager.yaml>] migrate parity --run-dir <dir> [--json|--yaml]
dotfiles-manager [--config <.dotfiles-manager.yaml>] migrate promote-preview --run-dir <dir> [--json]
```
...
## Example conventions and safety labels

Command examples in this reference use copy/pasteable commands without shell
prompt characters. Unless a section says otherwise:

- commands assume you are in the settings repository root;
- v2 examples pass `--config dotfiles-manager.v2.yaml` when scripts should be
  explicit about the active repository;
- examples using `docs-user`, `docs-machine`, `alice`, or example domains are
  synthetic and should be replaced with your own logical IDs and values;
- text output blocks show the expected shape, not a byte-for-byte promise for
  every version;
- `--dry-run`, `status`, `diff`, `list`, `recipe`, `backup show`, migration
  review helpers, and `app validate/test` are read-only unless a section states
  a generated fixture or output directory is written;
- `save --yes` writes desired state in the repository;
- `apply --yes` and `restore --yes` can write live config and should be preceded
  by their matching dry run;
- legacy `deploy` and `import` can write files and should be run with
  `--dry-run` first.

If an example writes persistent state, the surrounding section states what is
written and how to inspect, remove, or restore it.
...
`completion` prints shell-completion scripts generated by the CLI framework. It
does not read or write dotfiles-manager config by itself; writing the generated
script into a shell completion directory is the user's shell setup step.

Supported shells:

```text
dotfiles-manager completion bash [--no-descriptions]
dotfiles-manager completion fish [--no-descriptions]
dotfiles-manager completion powershell [--no-descriptions]
dotfiles-manager completion zsh [--no-descriptions]
```

Use each subcommand's `--help` output for shell-specific install locations:

```bash
dotfiles-manager completion zsh --help
```

Side effects:
- printing completion text to stdout is read-only;
- redirecting output into a shell completion directory writes wherever the shell
  documentation tells you to write it;
- completion scripts may expose command names and flags, but they do not include
...
### `migrate parity --run-dir <dir>`

`migrate parity` checks a generated migration run directory after `migrate` has
created reviewed v2 output under `migrations/v1-to-v2/<run-id>/generated/`. It
is a validation helper for migration review, not a live apply command.

```bash
dotfiles-manager migrate parity --run-dir migrations/v1-to-v2/<run-id>
dotfiles-manager migrate parity --run-dir migrations/v1-to-v2/<run-id> --json
dotfiles-manager migrate parity --run-dir migrations/v1-to-v2/<run-id> --yaml
```

Side effects:
- read-only;
- reads the migration run directory;
- emits text, JSON, or YAML diagnostics;
- does not write root v2 config, desired artifacts, or live files.

### `migrate promote-preview --run-dir <dir>`

`migrate promote-preview` previews optional promotion from generated
`custom.files` entries to known bundled targets when such promotion is safe to
consider. It is a review aid; it does not promote or apply anything by itself.

```bash
dotfiles-manager migrate promote-preview --run-dir migrations/v1-to-v2/<run-id>
dotfiles-manager migrate promote-preview --run-dir migrations/v1-to-v2/<run-id> --json
```

Side effects:
- read-only;
- reads the migration run directory;
- emits candidate promotion diagnostics;
- does not write root v2 config, desired artifacts, or live files.
...
desired artifacts, backups, run records, profile selections, native export/import
...
the real desired root, trust records, backups, or run records.
...
```bash
dotfiles-manager status --user-id alice git:user.email
dotfiles-manager save --dry-run --user-id alice git:user.email
dotfiles-manager save --yes --user-id alice git:user.email
dotfiles-manager diff --user-id alice git:user.email
dotfiles-manager apply --dry-run --user-id alice git:user.email
dotfiles-manager apply --yes --user-id alice git:user.email
```
...
For example, `--user-id alice` writes
`desired/user/alice/targets/git/settings.yaml`.
...
reports, run records, backup metadata, and JSON previews stay redacted.
...
backup under the manager's local state directory; normal output, run records, and
...
dotfiles-manager status --user-id alice starship:add_newline
dotfiles-manager save --dry-run --user-id alice starship:add_newline
dotfiles-manager save --yes --user-id alice starship:add_newline
dotfiles-manager diff --user-id alice starship:add_newline
dotfiles-manager apply --dry-run --user-id alice starship:add_newline
dotfiles-manager apply --yes --user-id alice starship:add_newline
```
...
For example, `--user-id alice` writes
`desired/user/alice/targets/starship/settings.yaml`.
...
dotfiles-manager status --user-id alice zsh:zshrc
dotfiles-manager save --dry-run --user-id alice zsh:zshrc
dotfiles-manager save --yes --user-id alice zsh:zshrc
dotfiles-manager diff --user-id alice zsh:zshrc
dotfiles-manager apply --dry-run --user-id alice zsh:zshrc
dotfiles-manager apply --yes --user-id alice zsh:zshrc
```
...
For example, `--user-id alice` and `zsh:zshrc` write:

```text
desired/user/alice/targets/zsh/artifacts/zshrc
```

The desired artifact contains the raw file bytes because it is the file that
will be applied later. Normal text and JSON output, diffs, run records, and backup
...
dotfiles-manager status --user-id alice tmux:home.conf
dotfiles-manager save --dry-run --user-id alice tmux:home.conf
dotfiles-manager save --yes --user-id alice tmux:home.conf
dotfiles-manager diff --user-id alice tmux:home.conf
dotfiles-manager apply --dry-run --user-id alice tmux:home.conf
dotfiles-manager apply --yes --user-id alice tmux:home.conf
```
...
For example, `--user-id alice` and `tmux:home.conf` write:

```text
desired/user/alice/targets/tmux/artifacts/home.conf
```
...
that will be applied later. Normal text and JSON output, diffs, run records, and
...
dotfiles-manager status --user-id alice ssh:config
dotfiles-manager save --dry-run --user-id alice ssh:config
dotfiles-manager save --yes --user-id alice ssh:config
dotfiles-manager diff --user-id alice ssh:config
dotfiles-manager apply --dry-run --user-id alice ssh:config
dotfiles-manager apply --yes --user-id alice ssh:config
```
...
```text
desired/user/alice/targets/ssh/artifacts/config
desired://user/alice/targets/ssh/artifacts/config
```
...
dotfiles-manager status --user-id alice nvim:config
dotfiles-manager save --dry-run --user-id alice nvim:config
dotfiles-manager save --yes --user-id alice nvim:config
dotfiles-manager diff --user-id alice nvim:config
dotfiles-manager apply --dry-run --user-id alice nvim:config
dotfiles-manager apply --yes --user-id alice nvim:config
```
...
```text
desired/user/alice/targets/nvim/artifacts/config
```
...
files that will be applied later. Normal text and JSON output, diffs, run records,
...
For example, a user-scoped `test.files:config` setting for `--user-id alice`
uses:

```text
desired/user/alice/targets/test.files/artifacts/config
```
...
```bash
dotfiles-manager status --user-id alice test.files:config
dotfiles-manager save --dry-run --user-id alice test.files:config
dotfiles-manager save --yes --user-id alice test.files:config
dotfiles-manager diff --user-id alice test.files:config
dotfiles-manager apply --dry-run --user-id alice test.files:config
dotfiles-manager apply --yes --user-id alice test.files:config
```
...
kind, and backup/run refs, but they do not print raw file contents. The
...
```bash
dotfiles-manager sync --json --user-id alice git:user.email
dotfiles-manager sync --choice git:user.email=save --yes --user-id alice git:user.email
```
