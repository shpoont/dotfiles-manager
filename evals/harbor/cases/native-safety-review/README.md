...
- internal pre-write recovery evidence and post-import verification policy;
- manager-owned temp/input/output roots;
- exit `5` or equivalent safety blocker/fail-closed behavior for
  unsupported, untrusted, secret, lifecycle, internal recovery-evidence, import, or
...
- allows `bash -c`, script strings, inherited environment, or `PATH`
  lookup;
- imports without required internal recovery evidence or treats that evidence as best effort;
...
- ledger/internal-recovery-evidence integration tests;
