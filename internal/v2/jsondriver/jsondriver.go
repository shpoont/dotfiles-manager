...
const NormalizerID = "json-file.selected-scalar.v1"
...
func (d Driver) Normalize(raw []byte) (State, error) {
	canonical, err := normalizeScalarJSON(raw)
	if err != nil {
		return State{}, err
	}
...
func validateSelector(selector Selector) error {
	...
	for idx, segment := range selector.Path {
		...
		if isExpressionSegment(segment) {
			return fmt.Errorf("selector path segment %d looks like an expression, not an object member", idx)
		}
	}
...
func selectScalar(root any, selector Selector) (selectedScalar, error) {
	...
	for idx, segment := range selector.Path {
		...
		if !ok {
			return selectedScalar{}, fmt.Errorf("selector path segment %q requires object container", selector.Path[idx])
		}
		...
		if idx == len(selector.Path)-1 {
			if !isJSONScalar(value) {
				return selectedScalar{}, fmt.Errorf("selected value at path %s must be a JSON scalar", strings.Join(selector.Path, "."))
			}
...
func setSelected(root any, selector Selector, value any) (any, error) {
	if !isJSONScalar(value) {
		return nil, fmt.Errorf("desired selected value must be a JSON scalar")
	}
	if root == nil {
		if selectorCreatePolicy(selector) != CreatePolicyCreate {
			return nil, fmt.Errorf("missing JSON document requires createMissing policy %q", CreatePolicyCreate)
		}
		root = map[string]any{}
	}
	current, ok := root.(map[string]any)
	if !ok {
		return nil, fmt.Errorf("selector root requires object document")
	}
	for idx, segment := range selector.Path {
		if idx == len(selector.Path)-1 {
			...
			if exists && !isJSONScalar(existing) {
				return nil, fmt.Errorf("existing selected value at path %s must be a JSON scalar", strings.Join(selector.Path, "."))
			}
		...
		if !ok {
			return nil, fmt.Errorf("selector path segment %q requires object container", segment)
		}
		current = child
	}
	return root, nil
...
func deleteSelected(root any, selector Selector) (any, error) {
	...
	for idx, segment := range selector.Path {
		if idx == len(selector.Path)-1 {
			if existing, exists := current[segment]; exists && !isJSONScalar(existing) {
				return nil, fmt.Errorf("selected value at path %s must be a JSON scalar", strings.Join(selector.Path, "."))
			}
...
func normalizeScalarJSON(raw []byte) ([]byte, error) {
	value, err := parseJSONDocument(raw)
	if err != nil {
		return nil, err
	}
	if !isJSONScalar(value) {
		return nil, fmt.Errorf("selected value must be a JSON scalar")
	}
...
func parseJSONDocument(data []byte) (any, error) {
	...
	if token, err := decoder.Token(); err == nil {
		return nil, fmt.Errorf("trailing JSON data after top-level value: %v", token)
	} else if err != io.EOF {
		return nil, err
	}
	return value, nil
...
func parseJSONValue(decoder *json.Decoder) (any, error) {
	...
	case '{':
		...
		for decoder.More() {
			...
			if !ok {
				return nil, fmt.Errorf("object key token is not a string")
			}
		...
		if end != json.Delim('}') {
			return nil, fmt.Errorf("expected object close delimiter, got %v", end)
		}
		return object, nil
	case '[':
		array := []any{}
		...
		if end != json.Delim(']') {
			return nil, fmt.Errorf("expected array close delimiter, got %v", end)
		}
		return array, nil
	default:
...
func writeFileAtomic(path string, data []byte) error {
	...
	tmp, err := os.CreateTemp(parent, ".dfm-json-*")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
