...
func TestWriteSelectedValueBackupPersistsMetadataAndPayload(t *testing.T) {
	...
	require.Equal(t, filedriver.Driver{}.Normalize([]byte("user:\n  email: old@example.com\n")).SHA256, item.Before.Hash)
	require.Equal(t, filedriver.NormalizerID, item.Before.Normalizer)
	...
	require.Equal(t, PlistFileSelectedDriverVersion, SelectedValueDriverVersion(recipe.PlistFileDriverID))
	require.Equal(t, "plist-file.selected-scalar.v1", SelectedValueNormalizer(recipe.PlistFileDriverID))
