...
`))
	...
	for idx, settingID := range starshipSettingIDs() {
		...
		require.Equal(t, "~/.config", starship.RecipeExplain.Resources[idx].LocationDefault)
		require.Equal(t, "~/.config/starship.toml", starship.RecipeExplain.Resources[idx].DisplayPath)
	}
	...
	require.Contains(t, starshipText, "Location: ~/.config/starship.toml (add_newline)")
	require.NotContains(t, starshipText, "$XDG_CONFIG_HOME")
	require.Contains(t, starshipText, "- STARSHIP_CONFIG or process XDG_CONFIG_HOME non-default locations")
	starshipVerbose := ExplainVerboseText(starship)
	require.Contains(t, starshipVerbose, "selector=add_newline")
	require.Contains(t, starshipVerbose, "do not manage: STARSHIP_CONFIG or process XDG_CONFIG_HOME non-default locations")
	...
	require.Equal(t, "~/.config", nvim.RecipeExplain.Resources[0].LocationDefault)
	require.Equal(t, "nvim", nvim.RecipeExplain.Resources[0].Path)
	require.Equal(t, "~/.config/nvim", nvim.RecipeExplain.Resources[0].DisplayPath)
...
func TestExplainRenderingAndHelperBranches(t *testing.T) {
	...
	resource := explainResource("tree", Resource{Driver: FileTreeDriverID, Location: "config", Path: "profiles", Include: []string{"**/*.json"}, Exclude: []string{"cache/**"}}, nil, nil)
	...
	resource = explainResource("raw", Resource{Driver: FileDriverID, Location: "config", Path: "config.txt"}, nil, nil)
	require.Equal(t, "file", resource.DiffMode)

	resource = explainResource("unknown", Resource{Driver: "custom-driver", Location: "config", Path: "config.txt"}, nil, nil)
...
func TestFriendlyExplainHelpersCoverFallbackBranches(t *testing.T) {
	...
	require.Equal(t, "~/.config/starship.toml", friendlyResourceLocation(ExplainResource{LocationID: "config", LocationDefault: "~/.config", Path: "/starship.toml"}))
	require.Equal(t, "config/starship.toml", friendlyResourceLocation(ExplainResource{LocationID: "config", Path: "/starship.toml"}))
