...
func TestBundledRegistryLookupAliasesAndDeterministicList(t *testing.T) {
	...
	require.Equal(t, ZshTarget, targets[4].ID)
	...
	require.Equal(t, "recipe://bundled/zsh", targets[4].RecipeRef)
	require.Equal(t, []string{CustomFilesTarget, GitTarget, NvimTarget, StarshipTarget, ZshTarget}, KnownBundledTargetIDs())
}
...
func TestRecipeListReportIsStableMetadataOnly(t *testing.T) {
	...
	require.Len(t, report.RecipeList.Targets, 5)
	...
	require.Equal(t, ZshTarget, report.RecipeList.Targets[4].ID)
