...
func TestBundledRegistryLookupAliasesAndDeterministicList(t *testing.T) {
	...
	require.Equal(t, StarshipTarget, targets[3].ID)
	require.Equal(t, TmuxTarget, targets[4].ID)
	require.Equal(t, ZshTarget, targets[5].ID)
	require.Equal(t, "recipe://bundled/nvim", targets[2].RecipeRef)
	require.Equal(t, "linux-darwin", targets[2].PlatformSupport)
	require.Equal(t, "recipe://bundled/starship", targets[3].RecipeRef)
	require.Equal(t, "unknown", targets[3].PlatformSupport)
	require.Equal(t, "recipe://bundled/tmux", targets[4].RecipeRef)
	require.Equal(t, "linux-darwin", targets[4].PlatformSupport)
	require.Equal(t, "recipe://bundled/zsh", targets[5].RecipeRef)
	require.Equal(t, []string{CustomFilesTarget, GitTarget, NvimTarget, StarshipTarget, TmuxTarget, ZshTarget}, KnownBundledTargetIDs())
}
...
func TestRecipeListReportIsStableMetadataOnly(t *testing.T) {
	...
	require.Len(t, report.RecipeList.Targets, 6)
	...
	require.Equal(t, StarshipTarget, report.RecipeList.Targets[3].ID)
	require.Equal(t, TmuxTarget, report.RecipeList.Targets[4].ID)
	require.Equal(t, ZshTarget, report.RecipeList.Targets[5].ID)
